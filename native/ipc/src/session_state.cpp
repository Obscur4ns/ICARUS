...
namespace
{
using icarus::core::SessionStatePage;
...
bool write_snapshot(
    icarus::core::SessionStateSlot<Payload>& slot,
...
template<typename Payload>
icarus::ipc::StateSnapshot<Payload> read_snapshot(
    const icarus::core::SessionStateSlot<Payload>& slot
) noexcept
{
    for(int attempt = 0; attempt < 5; ++attempt)
    {
        ...
        icarus::ipc::StateSnapshot<Payload> snapshot{};
...
{
class SessionStateChannel::Impl
{
public:
    ...
    bool publish_arma(const core::ArmaSessionState& state)
    {
        std::scoped_lock lock(mutex_);
    ...
    bool publish_voice_backend(const core::VoiceBackendSessionState& state)
    {
        std::scoped_lock lock(mutex_);
    ...
    StateSnapshot<core::ArmaSessionState> read_arma() const
    {
        std::scoped_lock lock(mutex_);
    ...
    StateSnapshot<core::VoiceBackendSessionState> read_voice_backend() const
    {
        std::scoped_lock lock(mutex_);
...
private:
    bool create_arma_mapping(std::uint64_t generation)
    {
        mapping_ = CreateFileMappingW(
            ...
            PAGE_READWRITE,
            0,
            static_cast<DWORD>(sizeof(core::SessionStatePage)),
            mapping_name_.c_str()
        );
        ...
        page_ = static_cast<core::SessionStatePage*>(
            MapViewOfFile(
                mapping_,
                FILE_MAP_ALL_ACCESS,
                0,
                0,
                sizeof(core::SessionStatePage)
            )
        );
        ...
        std::memset(page_, 0, sizeof(core::SessionStatePage));

        page_->header.magic = core::session_state_magic;
        page_->header.protocol_major = core::session_state_protocol_major;
        page_->header.protocol_minor = core::session_state_protocol_minor;
        page_->header.struct_size = static_cast<std::uint32_t>(sizeof(core::SessionStatePage));
    ...
    bool page_compatible(std::uint64_t generation) const noexcept
    {
        ...
        return page_->header.magic == core::session_state_magic
            && page_->header.protocol_major == core::session_state_protocol_major
            && page_->header.struct_size >= sizeof(core::SessionStatePage)
    ...
    HANDLE mapping_{};
    core::SessionStatePage* page_{};
    std::uint64_t generation_{};

    mutable std::mutex mutex_;
};

SessionStateChannel::SessionStateChannel(BridgeRole role, std::wstring mapping_name)
    : impl_(std::make_unique<Impl>(role, std::move(mapping_name)))
{
}

SessionStateChannel::~SessionStateChannel() = default;

bool SessionStateChannel::sync(std::uint64_t generation)
{
    return impl_->sync(generation);
}

void SessionStateChannel::reset()
{
    impl_->reset();
}

std::uint64_t SessionStateChannel::generation() const
{
    return impl_->generation();
}

bool SessionStateChannel::publish_arma(const core::ArmaSessionState& state)
{
    return impl_->publish_arma(state);
}

bool SessionStateChannel::publish_voice_backend(const core::VoiceBackendSessionState& state)
{
    return impl_->publish_voice_backend(state);
}

StateSnapshot<core::ArmaSessionState> SessionStateChannel::read_arma() const
{
    return impl_->read_arma();
}

StateSnapshot<core::VoiceBackendSessionState> SessionStateChannel::read_voice_backend() const
{
